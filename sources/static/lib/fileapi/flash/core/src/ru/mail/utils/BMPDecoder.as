package ru.mail.utils
{
		...
		public class BMPDecoder {
			...
			private var _decodeError:String;
			...
			public function decode( data:ByteArray ):BitmapData {
				...
				switch ( nBitsPerPixel ){
					...
					default:
						...
						trace ("BMPDecoder invalid bits per pixel : " + nBitsPerPixel);
			...
			private function readFileHeader():void {
				...
				try {
					...
					if ( fileHeader.readUTFBytes( 2 ) != BITMAP_HEADER_TYPE ){
						...
						trace ("BMPDecoder invalid bitmap header type");
					}
					...
					trace ("BMPDecoder invalid file header");
				}
			}
			/**
			...
			private function readInfoHeader():void {
				...
				try {
					...
					trace ("BMPDecoder invalid info header");
				}
			}
			/**
			...
			private function readBitFields():void {
				if ( nCompression == COMP_RGB ){
					...
					try {
						...
						trace ("BMPDecoder invalid bit fields");
						_decodeError = "BMPDecoder invalid bit fields";
					}
				}
			}
			/**
			...
			private function decode1BitBMP():void {
				...
				try {
					...
					trace ("BMPDecoder invalid image data");
					_decodeError = "BMPDecoder invalid image data";
				}
			}
			/**
			 * decode 4bit RLE
			...
			/**
			 * report if there were some errors while decoding 
			 * @return 
			 * 
			 */			
			public function get decodeError():String {
				return _decodeError;
			}
		}
	}