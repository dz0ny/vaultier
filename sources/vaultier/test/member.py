...
from rest_framework.status import HTTP_200_OK, HTTP_403_FORBIDDEN, HTTP_400_BAD_REQUEST, HTTP_204_NO_CONTENT, HTTP_404_NOT_FOUND
from vaultier.models.fields import RoleLevelField, MemberStatusField
...
from vaultier.test.invitation_tools import accept_invitation_api_call, retrieve_invitation_api_call
from vaultier.test.member_tools import invite_member_api_call, delete_member_api_call, list_members_api_call
...
from vaultier.test.workspacekey_tools import set_workspace_key_api_call, get_workspace_key_api_call
...
class ApiInviteTest(TransactionTestCase):
    ...
    def test_030_list_roles_for_hash(self):
        ...
        response = retrieve_invitation_api_call(user2token, user2member.get('id'), 'testovaci-hash' )
        self.assertEquals(
            response.status_code,
            HTTP_404_NOT_FOUND,
            format_response(response)
        ...
        response = retrieve_invitation_api_call(user2token, user2member.get('id'),user2invitation )
        self.assertEquals(
            response.status_code,
            HTTP_200_OK,
            format_response(response)
