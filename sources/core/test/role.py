...
class ApiRoleTest(TransactionTestCase):
    ...
    def test_role_is_saved_only_once(self):
        ...
        response = create_role_api_call(user1token, user2member.get('id'), level=AclLevelField.LEVEL_WRITE, to_workspace=workspace1)
        self.assertEqual(
        ...
        # user1 grant read role to user2
        response = create_role_api_call(user1token, user2member.get('id'), level=AclLevelField.LEVEL_READ, to_workspace=workspace1)
        self.assertEqual(
            response.status_code,
            HTTP_200_OK,
            format_response(response)
        ...
        roles = Role.objects.filter(member=user2member.get('id'), workspace=workspace1.get('id'))
        self.assertEqual(
            roles.count(),
            1,
            format_response(response)
