...
def cant_observe():
    ...
    if sys.platform != "linux2":
        blockers.append('Available only on Linux platform with '
...
def is_outdated(src_file, compiled_file):
    return not os.path.exists(compiled_file) or \
        int(os.stat(compiled_file).st_mtime * 1000) < \
        int(os.stat(src_file).st_mtime * 1000)
