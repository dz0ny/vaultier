...
export default Ember.Object.extend({
    ...
    switchToVisibleAllNodes: function () {
        if (!this.fullVisibilityOfTree) {
            this.nodes.forEach(function (node) {
                ...
                } else if (node.get('type') !== Node.proto().types.FOLDER.value) {
                    node.set('skipShow', false);
                }
            });
        }
    ...
    getRootNodeForNode: function (treeNode) {
        return treeNode.get('parent') ? this.getRootNodeForNode(treeNode.get('parent')) : treeNode;
    },
    ...
    _createRootNode: function (model) {
        return Node.create({
            children: [],
            type: Node.prototype.types.FOLDER.value,
            isOpened: false,
            content: model
        });
    },
    ...
    checkIfChildNodesAreLoaded: function (node) {
        ...
        node.get('children').forEach(function (childNode) {
            ...
            if (childNode.get('isLoaded') === false && childNode.get('type') === Node.proto().types.FOLDER.value) {
        ...
        return this.get('store')
            ...
            .then(function (modelNodes) {
                ...
                treeNodes.forEach(function (treeNode) {
                    if (treeNode.get('membership.status') !== Member.proto().statuses['MEMBER'].value) {
                        return;
                    }
    ...
    _insertNodeOnRightPlace: function (treeNode, parentTreeNode) {
        ...
        var indexToInputNode = this.nodes.get('content').indexOf(parentTreeNode) + childrenCount;
